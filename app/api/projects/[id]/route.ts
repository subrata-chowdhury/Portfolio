...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `https://api.github.com/repos/subrata-chowdhury/${id}`,
      {
    ...
    while (noOfCommits === null || noOfCommits === 100) {
      const response = await fetch(
        `https://api.github.com/repos/subrata-chowdhury/${id}/commits?per_page=100&page=${pageCount}`,
        {
          headers: { Authorization: "Bearer " + process.env.GITHUB_AUTH_TOKEN },
        },
      );
      if (response.status === 200) {
        ...
        createdAt = new Date(
          commitDetails[(await noOfCommits) - 1].commit.committer.date,
        ).toUTCString();
      } else {
        throw response.body;
      }
    ...
    return NextResponse.json(
      { noOfCommits: totalNoOfCommits, updatedAt, createdAt, description },
      { status: 200 },
    );
  } catch (error) {
    console.log(error);
